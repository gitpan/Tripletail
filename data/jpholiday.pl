...
BEGIN {
	if (!-e getcwd . "/lib/TL.pm") {
		print "$0: please run this script at the same directory as Makefile.PL.\n";
		exit 1;
	}

	# TL::DateTime::JPHoliday が既に読まれた事にする。
	eval q{
		package TL::DateTime::JPHoliday;
		our %HOLIDAY = ();
	};
	$INC{'TL/DateTime/JPHoliday.pm'} = 'Imitated by jpholiday.pl';
}
...
mkpath 'lib/TL/DateTime', 1;
print "Writing lib/TL/DateTime/JPHoliday.pm...\n";

open my $fh, '>', 'lib/TL/DateTime/JPHoliday.pm' or die $!;
print $fh $t->toStr;
